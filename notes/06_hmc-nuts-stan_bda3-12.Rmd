...
- random walk of Gibbs sampler and Metropolis algorthim is inherently inefficient
- Hamiltonian Monte Carlo (HMC) uses "mommentum" to suppress the local random walk behavior of the Metropolis algorithm
  - such that is moves more rapidly through the target distribution
  - for each compoent $\theta_j$ in the target space, there is a corresponding *momentum* variable $\phi_j$
...
- common to usea multivariate normal distribution with mean 0 and a diagonal *mass matrix* $M$
  - $\phi_j \sim \text{N}(0, M_{jj})$ for each $j = 1, \dots, d$
  - ideally, the mass matrix should scale with the inverse covaraince matrix of the posterior distribution $(\text{var}(\theta|y))^{-1}$

##### The three steps of an HMC iteration

1. update $\phi$ by sampling from its posterior (same as its prior): $\phi \sim \text{N}(0, M)$
2. simultaneously update $\theta$ and $\phi$ using a leapfrog algorith to simulate physical Hamilotnian dynaimcs where the position and momentum evolve continuously; for $L$ leapfrog steps with scaling factor $\epsilon$:
  ...
  b. use the momentum $\phi$ to update postition $\theta$: $\theta \leftarrow \theta + \epsilon M^{-1} \phi$
...
4. set $\theta^t = \theta^*$ with probability $\min(r, 1)$, else reject the prposed $\theta$ and set $\theta^t = \theta^{t-1}$
...
**NEXT -> watch video on HMC by Betancourt: https://www.youtube.com/watch?v=jUSZboSq1zg**
